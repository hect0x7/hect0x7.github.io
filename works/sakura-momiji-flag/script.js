...
const transitionVariant = 'frame';
const introLayouts = {
    frame: { width: .56, left: .055, copyLeft: .68, copyWidth: .27 }
};
const introLayout = introLayouts[transitionVariant];
document.body.dataset.transitionVariant = transitionVariant;
...
function renderIntro(progress) {
    ...
    const height = h + (target.height - h) * morph;
    const left = target.left * morph;
    const center = h / 2 + (target.center - h / 2) * morph;
    introVisual.style.width = `${width}px`;
    introVisual.style.height = `${height}px`;
    introVisual.style.left = `${left}px`;
    introVisual.style.top = `${center}px`;
    introCopy.style.top = `${target.copyTop}px`;

    let clearance;
    if (target.below) clearance = target.copyTop - center - height / 2;
    ...
    introCopy.style.opacity = String(reveal);
    introCopy.style.transform = `translate3d(0, ${(1 - reveal) * 20}px, 0)`;
}
...
function setActive(index, localProgress) {
    ...
    document.querySelectorAll('.progress-fill').forEach((item, itemIndex) => {
        const progress = itemIndex === index ? clamp(localProgress) : 0;
...
function update() {
    ...
    if (raw < INTRO_PHASE) {
        renderIntro(raw / INTRO_PHASE);
    } else {
        const target = introTargetMetrics();
        introVisual.style.width = `${target.width}px`;
        introVisual.style.height = `${target.height}px`;
        introVisual.style.left = `${target.left}px`;
        introVisual.style.top = `${target.center}px`;
        introCopy.style.top = `${target.copyTop}px`;
...
const en = {
    ...
    arrive: 'Explore seasons', scrollHint: 'Scroll to explore',
